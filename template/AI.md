...
- The package exports one class: `{Class}`.
- Every `{Class}.MethodName(...)` call is a thin 1:1 binding to `{name}.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `{Class}.Preload()` only changes when symbols are bound. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `{Class}` class and re-exports everything from `types/{Class}.ts`.
- `structs/{Class}.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/{name}` when available.
