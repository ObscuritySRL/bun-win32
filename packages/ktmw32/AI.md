...
```ts
import Ktmw32, { SomeFlag } from '@bun-win32/ktmw32';

// Methods bind lazily on first call
const result = Ktmw32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Ktmw32.Preload(['SomeFunctionW', 'AnotherFunction']);
Ktmw32.Preload('SomeFunctionW');
Ktmw32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Ktmw32.ts` |
| Find types, enums, constants      | `types/Ktmw32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/ktmw32` directly.

## Calling Convention

All documented `ktmw32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
