# AI Guide for @bun-win32/mmdevapi
...
```ts
import Mmdevapi, { SomeFlag } from '@bun-win32/mmdevapi';

// Methods bind lazily on first call
const result = Mmdevapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mmdevapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Mmdevapi.Preload('SomeFunctionW');
Mmdevapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Mmdevapi.ts` |
| Find types, enums, constants      | `types/Mmdevapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/mmdevapi` directly.

## Calling Convention

All documented `mmdevapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Mmdevapi.SomeFunctionW(wide.ptr);
