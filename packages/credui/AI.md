...
```ts
import Credui, { SomeFlag } from '@bun-win32/credui';

// Methods bind lazily on first call
const result = Credui.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Credui.Preload(['SomeFunctionW', 'AnotherFunction']);
Credui.Preload('SomeFunctionW');
Credui.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Credui.ts` |
| Find types, enums, constants      | `types/Credui.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types - import from `@bun-win32/credui` directly.

## Calling Convention

All documented `credui.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
