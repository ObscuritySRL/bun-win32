...
```ts
import Dxcore, { SomeFlag } from '@bun-win32/dxcore';

// Methods bind lazily on first call
const result = Dxcore.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dxcore.Preload(['SomeFunctionW', 'AnotherFunction']);
Dxcore.Preload('SomeFunctionW');
Dxcore.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dxcore.ts` |
| Find types, enums, constants      | `types/Dxcore.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dxcore` directly.

## Calling Convention

All documented `dxcore.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
