# AI Guide for @bun-win32/kernel32

This file documents the binding contract of this package, not the Windows API itself.

## What This Package Is

- The package exports one class: `Kernel32`.
- Every `Kernel32.MethodName(...)` call is a thin 1:1 binding to `kernel32.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `Kernel32.Preload()` is only a bind-time optimization. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `Kernel32` class and re-exports everything from `types/Kernel32.ts`.
- `structs/Kernel32.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
- `types/Kernel32.ts` contains the aliases, enums, constants, flags, and sentinels used by the method signatures.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/kernel32` when available.

## Errors And Lifetime

- Return values are raw native results.
- Failure sentinels and error sources follow the underlying API contract.
- When the native API uses Win32 last-error semantics, read the error through the corresponding API, such as `Kernel32.GetLastError()`.
