...
```ts
import Kernel32, { SomeFlag } from '@bun-win32/kernel32';

// Methods bind lazily on first call
const result = Kernel32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Kernel32.Preload(['SomeFunctionW', 'AnotherFunction']);
Kernel32.Preload('SomeFunctionW');
Kernel32.Preload();
```
...
| Need                              | Read                  |
| --------------------------------- | --------------------- |
| Find a method or its MS Docs link | `structs/Kernel32.ts` |
| Find types, enums, constants      | `types/Kernel32.ts`   |
| Quick examples                    | `README.md`           |

`index.ts` re-exports the class and all types — import from `@bun-win32/kernel32` directly.

## Calling Convention

All documented `kernel32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Kernel32.SomeFunctionW(wide.ptr);
