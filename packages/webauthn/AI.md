# AI Guide for @bun-win32/webauthn
...
```ts
import Webauthn, { SomeFlag } from '@bun-win32/webauthn';

// Methods bind lazily on first call
const result = Webauthn.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Webauthn.Preload(['SomeFunctionW', 'AnotherFunction']);
Webauthn.Preload('SomeFunctionW');
Webauthn.Preload();
```
...
| Need                              | Read                  |
| --------------------------------- | --------------------- |
| Find a method or its MS Docs link | `structs/Webauthn.ts` |
| Find types, enums, constants      | `types/Webauthn.ts`   |
| Quick examples                    | `README.md`           |

`index.ts` re-exports the class and all types — import from `@bun-win32/webauthn` directly.

## Calling Convention

All documented `webauthn.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Webauthn.SomeFunctionW(wide.ptr);
