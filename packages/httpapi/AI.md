# AI Guide for @bun-win32/httpapi
...
```ts
import Httpapi, { SomeFlag } from '@bun-win32/httpapi';

// Methods bind lazily on first call
const result = Httpapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Httpapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Httpapi.Preload('SomeFunctionW');
Httpapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Httpapi.ts` |
| Find types, enums, constants      | `types/Httpapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/httpapi` directly.

## Calling Convention

All documented `httpapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Httpapi.SomeFunctionW(wide.ptr);
