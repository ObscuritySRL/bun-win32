# AI Guide for @bun-win32/uxtheme
...
```ts
import Uxtheme, { SomeFlag } from '@bun-win32/uxtheme';

// Methods bind lazily on first call
const result = Uxtheme.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Uxtheme.Preload(['SomeFunctionW', 'AnotherFunction']);
Uxtheme.Preload('SomeFunctionW');
Uxtheme.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Uxtheme.ts` |
| Find types, enums, constants      | `types/Uxtheme.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types - import from `@bun-win32/uxtheme` directly.

## Calling Convention

All documented `uxtheme.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Uxtheme.SomeFunctionW(wide.ptr);
