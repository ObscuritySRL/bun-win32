...
```ts
import Iphlpapi, { SomeFlag } from '@bun-win32/iphlpapi';

// Methods bind lazily on first call
const result = Iphlpapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Iphlpapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Iphlpapi.Preload('SomeFunctionW');
Iphlpapi.Preload();
```
...
| Need                              | Read                  |
| --------------------------------- | --------------------- |
| Find a method or its MS Docs link | `structs/Iphlpapi.ts` |
| Find types, enums, constants      | `types/Iphlpapi.ts`   |
| Quick examples                    | `README.md`           |

`index.ts` re-exports the class and all types — import from `@bun-win32/iphlpapi` directly.

## Calling Convention

All documented `iphlpapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Iphlpapi.SomeFunctionW(wide.ptr);
