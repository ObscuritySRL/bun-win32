# AI Guide for @bun-win32/iphlpapi

This file documents the binding contract of this package, not the Windows API itself.

## What This Package Is

- The package exports one class: `Iphlpapi`.
- Every `Iphlpapi.MethodName(...)` call is a thin 1:1 binding to `iphlpapi.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `Iphlpapi.Preload()` only changes when symbols are bound. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `Iphlpapi` class and re-exports everything from `types/Iphlpapi.ts`.
- `structs/Iphlpapi.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
- `types/Iphlpapi.ts` contains the aliases, enums, constants, flags, and sentinels used by the method signatures.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/iphlpapi` when available.
