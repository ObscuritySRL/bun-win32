...
```ts
import Amsi, { SomeFlag } from '@bun-win32/amsi';

// Methods bind lazily on first call
const result = Amsi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Amsi.Preload(['SomeFunctionW', 'AnotherFunction']);
Amsi.Preload('SomeFunctionW');
Amsi.Preload();
```
...
| Need                              | Read              |
| --------------------------------- | ----------------- |
| Find a method or its MS Docs link | `structs/Amsi.ts` |
| Find types, enums, constants      | `types/Amsi.ts`   |
| Quick examples                    | `README.md`       |

`index.ts` re-exports the class and all types — import from `@bun-win32/amsi` directly.

## Calling Convention

All documented `amsi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
