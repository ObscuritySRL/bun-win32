# AI Guide for @bun-win32/wininet
...
```ts
import Wininet, { SomeFlag } from '@bun-win32/wininet';

// Methods bind lazily on first call
const result = Wininet.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wininet.Preload(['SomeFunctionW', 'AnotherFunction']);
Wininet.Preload('SomeFunctionW');
Wininet.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Wininet.ts` |
| Find types, enums, constants      | `types/Wininet.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/wininet` directly.

## Calling Convention

All documented `wininet.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Wininet.SomeFunctionW(wide.ptr);
