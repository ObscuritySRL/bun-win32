...
```ts
import Shcore, { SomeFlag } from '@bun-win32/shcore';

// Methods bind lazily on first call
const result = Shcore.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Shcore.Preload(['SomeFunctionW', 'AnotherFunction']);
Shcore.Preload('SomeFunctionW');
Shcore.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Shcore.ts` |
| Find types, enums, constants      | `types/Shcore.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/shcore` directly.

## Calling Convention

All documented `shcore.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
