# AI Guide for @bun-win32/wimgapi
...
```ts
import Wimgapi, { SomeFlag } from '@bun-win32/wimgapi';

// Methods bind lazily on first call
const result = Wimgapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wimgapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Wimgapi.Preload('SomeFunctionW');
Wimgapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Wimgapi.ts` |
| Find types, enums, constants      | `types/Wimgapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/wimgapi` directly.

## Calling Convention

All documented `wimgapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Wimgapi.SomeFunctionW(wide.ptr);
