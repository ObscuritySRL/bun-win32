...
```ts
import Fltlib, { SomeFlag } from '@bun-win32/fltlib';

// Methods bind lazily on first call
const result = Fltlib.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Fltlib.Preload(['SomeFunctionW', 'AnotherFunction']);
Fltlib.Preload('SomeFunctionW');
Fltlib.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Fltlib.ts` |
| Find types, enums, constants      | `types/Fltlib.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/fltlib` directly.

## Calling Convention

All documented `fltlib.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
