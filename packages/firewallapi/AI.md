# AI Guide for @bun-win32/firewallapi
...
```ts
import FirewallApi, { SomeFlag } from '@bun-win32/firewallapi';

// Methods bind lazily on first call
const result = FirewallApi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
FirewallApi.Preload(['SomeFunctionW', 'AnotherFunction']);
FirewallApi.Preload('SomeFunctionW');
FirewallApi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/FirewallApi.ts` |
| Find types, enums, constants      | `types/FirewallApi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/firewallapi` directly.

## Calling Convention

All documented `firewallapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
FirewallApi.SomeFunctionW(wide.ptr);
...
FirewallApi.SomeFunction(out.ptr);
