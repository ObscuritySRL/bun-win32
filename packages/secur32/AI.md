...
```ts
import Secur32, { SomeFlag } from '@bun-win32/secur32';

// Methods bind lazily on first call
const result = Secur32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Secur32.Preload(['SomeFunctionW', 'AnotherFunction']);
Secur32.Preload('SomeFunctionW');
Secur32.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/Secur32.ts`   |
| Find types, enums, constants      | `types/Secur32.ts`     |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/secur32` directly.

## Calling Convention

All documented `secur32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Secur32.SomeFunctionW(wide.ptr);
