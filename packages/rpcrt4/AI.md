...
```ts
import Rpcrt4, { RpcAuthnLevel } from '@bun-win32/rpcrt4';

// Methods bind lazily on first call
const result = Rpcrt4.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Rpcrt4.Preload(['SomeFunctionW', 'AnotherFunction']);
Rpcrt4.Preload('SomeFunctionW');
Rpcrt4.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Rpcrt4.ts` |
| Find types, enums, constants      | `types/Rpcrt4.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/rpcrt4` directly.

## Calling Convention

All documented `rpcrt4.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
