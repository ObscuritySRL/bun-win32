# AI Guide for @bun-win32/magnification
...
```ts
import Magnification, { SomeFlag } from '@bun-win32/magnification';

// Methods bind lazily on first call
const result = Magnification.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Magnification.Preload(['SomeFunctionW', 'AnotherFunction']);
Magnification.Preload('SomeFunctionW');
Magnification.Preload();
```
...
| Need                              | Read                       |
| --------------------------------- | -------------------------- |
| Find a method or its MS Docs link | `structs/Magnification.ts` |
| Find types, enums, constants      | `types/Magnification.ts`   |
| Quick examples                    | `README.md`                |

`index.ts` re-exports the class and all types — import from `@bun-win32/magnification` directly.

## Calling Convention

All documented `magnification.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Magnification.SomeFunctionW(wide.ptr);
...
Magnification.SomeFunction(out.ptr);
