...
```ts
import Crypt32, { SomeFlag } from '@bun-win32/crypt32';

// Methods bind lazily on first call
const result = Crypt32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Crypt32.Preload(['SomeFunctionW', 'AnotherFunction']);
Crypt32.Preload('SomeFunctionW');
Crypt32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Crypt32.ts` |
| Find types, enums, constants      | `types/Crypt32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/crypt32` directly.

## Calling Convention

All documented `crypt32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Crypt32.SomeFunctionW(wide.ptr);
