...
const pngChunk = (type: string, data: Uint8Array): Uint8Array => {
  ...
  chunk.set(body, 4);
  chunk.set(uint32BigEndian(Bun.hash.crc32(body) >>> 0), 4 + body.length); // native: 56× the per-byte JS loop at 1080p sizes
  return chunk;
};
...
export const encodePNG = (rgbPixels: Uint8Array, width: number, height: number): Uint8Array => {
  ...
  zlib[1] = 0x01;
  zlib.set(deflated, 2);
  zlib.set(uint32BigEndian(Bun.hash.adler32(filtered) >>> 0), 2 + deflated.length);
...
/** Encode a tightly packed width×height RGBA8 buffer (the readbackTexture layout) to a PNG byte array — alpha preserved (color type 6). */
export const encodePNGFromRGBA = (rgbaPixels: Uint8Array, width: number, height: number): Uint8Array => {
  const scanlineLength = 1 + width * 4;
  const filtered = new Uint8Array(height * scanlineLength);
  for (let row = 0; row < height; row++) {
    filtered[row * scanlineLength] = 0;
    filtered.set(rgbaPixels.subarray(row * width * 4, (row + 1) * width * 4), row * scanlineLength + 1);
  }
  const deflated = Bun.deflateSync(filtered);
  const zlib = new Uint8Array(2 + deflated.length + 4);
  zlib[0] = 0x78;
