# AI Guide for @bun-win32/comdlg32
...
```ts
import Comdlg32, { SomeFlag } from '@bun-win32/comdlg32';

// Methods bind lazily on first call
const result = Comdlg32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Comdlg32.Preload(['SomeFunctionW', 'AnotherFunction']);
Comdlg32.Preload('SomeFunctionW');
Comdlg32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Comdlg32.ts` |
| Find types, enums, constants      | `types/Comdlg32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/comdlg32` directly.

## Calling Convention

All documented `comdlg32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Comdlg32.SomeFunctionW(wide.ptr);
