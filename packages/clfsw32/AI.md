# AI Guide for @bun-win32/clfsw32
...
```ts
import Clfsw32, { SomeFlag } from '@bun-win32/clfsw32';

// Methods bind lazily on first call
const result = Clfsw32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Clfsw32.Preload(['SomeFunctionW', 'AnotherFunction']);
Clfsw32.Preload('SomeFunctionW');
Clfsw32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Clfsw32.ts` |
| Find types, enums, constants      | `types/Clfsw32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/clfsw32` directly.

## Calling Convention

All documented `clfsw32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Clfsw32.SomeFunctionW(wide.ptr);
