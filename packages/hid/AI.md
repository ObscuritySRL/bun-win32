...
```ts
import Hid, { SomeFlag } from '@bun-win32/hid';

// Methods bind lazily on first call
const result = Hid.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Hid.Preload(['SomeFunctionW', 'AnotherFunction']);
Hid.Preload('SomeFunctionW');
Hid.Preload();
```
...
| Need                              | Read             |
| --------------------------------- | ---------------- |
| Find a method or its MS Docs link | `structs/Hid.ts` |
| Find types, enums, constants      | `types/Hid.ts`   |
| Quick examples                    | `README.md`      |

`index.ts` re-exports the class and all types — import from `@bun-win32/hid` directly.

## Calling Convention

All documented `hid.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
