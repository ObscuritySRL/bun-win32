...
```ts
import Mf, { SomeFlag } from '@bun-win32/mf';

// Methods bind lazily on first call
const result = Mf.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mf.Preload(['SomeFunctionW', 'AnotherFunction']);
Mf.Preload('SomeFunctionW');
Mf.Preload();
```
...
| Need                              | Read              |
| --------------------------------- | ----------------- |
| Find a method or its MS Docs link | `structs/Mf.ts`   |
| Find types, enums, constants      | `types/Mf.ts`     |
| Quick examples                    | `README.md`       |

`index.ts` re-exports the class and all types — import from `@bun-win32/mf` directly.

## Calling Convention

All documented `mf.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
