# AI Guide for @bun-win32/prntvpt
...
```ts
import Prntvpt, { SomeFlag } from '@bun-win32/prntvpt';

// Methods bind lazily on first call
const result = Prntvpt.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Prntvpt.Preload(['SomeFunctionW', 'AnotherFunction']);
Prntvpt.Preload('SomeFunctionW');
Prntvpt.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Prntvpt.ts` |
| Find types, enums, constants      | `types/Prntvpt.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/prntvpt` directly.

## Calling Convention

All documented `prntvpt.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Prntvpt.SomeFunctionW(wide.ptr);
