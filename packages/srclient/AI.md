# AI Guide for @bun-win32/srclient
...
```ts
import Srclient, { SomeFlag } from '@bun-win32/srclient';

// Methods bind lazily on first call
const result = Srclient.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Srclient.Preload(['SomeFunctionW', 'AnotherFunction']);
Srclient.Preload('SomeFunctionW');
Srclient.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Srclient.ts` |
| Find types, enums, constants      | `types/Srclient.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/srclient` directly.

## Calling Convention

All documented `srclient.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Srclient.SomeFunctionW(wide.ptr);
