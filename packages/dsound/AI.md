...
```ts
import DSound, { SomeFlag } from '@bun-win32/dsound';

// Methods bind lazily on first call
const result = DSound.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
DSound.Preload(['SomeFunctionW', 'AnotherFunction']);
DSound.Preload('SomeFunctionW');
DSound.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/DSound.ts` |
| Find types, enums, constants      | `types/DSound.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/dsound` directly.

## Calling Convention

All documented `dsound.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
