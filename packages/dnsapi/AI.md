...
```ts
import Dnsapi, { SomeFlag } from '@bun-win32/dnsapi';

// Methods bind lazily on first call
const result = Dnsapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dnsapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Dnsapi.Preload('SomeFunctionW');
Dnsapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dnsapi.ts` |
| Find types, enums, constants      | `types/Dnsapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dnsapi` directly.

## Calling Convention

All documented `dnsapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
