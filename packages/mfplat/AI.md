...
```ts
import Mfplat, { SomeFlag } from '@bun-win32/mfplat';

// Methods bind lazily on first call
const result = Mfplat.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mfplat.Preload(['SomeFunctionW', 'AnotherFunction']);
Mfplat.Preload('SomeFunctionW');
Mfplat.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Mfplat.ts` |
| Find types, enums, constants      | `types/Mfplat.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/mfplat` directly.

## Calling Convention

All documented `mfplat.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
