...
```ts
import Mscms, { PROFILE_ACCESS } from '@bun-win32/mscms';

// Methods bind lazily on first call
const result = Mscms.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mscms.Preload(['SomeFunctionW', 'AnotherFunction']);
Mscms.Preload('SomeFunctionW');
Mscms.Preload();
```
...
| Need                              | Read               |
| --------------------------------- | ------------------ |
| Find a method or its MS Docs link | `structs/Mscms.ts` |
| Find types, enums, constants      | `types/Mscms.ts`   |
| Quick examples                    | `README.md`        |

`index.ts` re-exports the class and all types — import from `@bun-win32/mscms` directly.

## Calling Convention

All documented `mscms.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
- `HANDLE`, `HPROFILE`, `HTRANSFORM` → `bigint`
- `DWORD`, `UINT`, `BOOL`, `INT`, `LONG`, `HRESULT` → `number`
...
- **Handle** params (`HPROFILE`, `HTRANSFORM`, etc.): pass a `bigint` value.
