...
- The package exports one class: `Shlwapi`.
- Every `Shlwapi.MethodName(...)` call is a thin 1:1 binding to `shlwapi.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `Shlwapi.Preload()` only changes when symbols are bound. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `Shlwapi` class and re-exports everything from `types/Shlwapi.ts`.
- `structs/Shlwapi.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
- `types/Shlwapi.ts` contains the aliases, enums, constants, flags, and sentinels used by the method signatures.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/shlwapi` when available.
