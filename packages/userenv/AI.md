# AI Guide for @bun-win32/userenv
...
```ts
import Userenv, { SomeFlag } from '@bun-win32/userenv';

// Methods bind lazily on first call
const result = Userenv.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Userenv.Preload(['SomeFunctionW', 'AnotherFunction']);
Userenv.Preload('SomeFunctionW');
Userenv.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Userenv.ts` |
| Find types, enums, constants      | `types/Userenv.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/userenv` directly.

## Calling Convention

All documented `userenv.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Userenv.SomeFunctionW(wide.ptr);
