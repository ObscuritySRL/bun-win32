# AI Guide for @bun-win32/clusapi
...
```ts
import Clusapi, { SomeFlag } from '@bun-win32/clusapi';

// Methods bind lazily on first call
const result = Clusapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Clusapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Clusapi.Preload('SomeFunctionW');
Clusapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Clusapi.ts` |
| Find types, enums, constants      | `types/Clusapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/clusapi` directly.

## Calling Convention

All documented `clusapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Clusapi.SomeFunctionW(wide.ptr);
