# AI Guide for @bun-win32/taskschd
...
```ts
import Taskschd, { SomeFlag } from '@bun-win32/taskschd';

// Methods bind lazily on first call
const result = Taskschd.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Taskschd.Preload(['SomeFunctionW', 'AnotherFunction']);
Taskschd.Preload('SomeFunctionW');
Taskschd.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Taskschd.ts` |
| Find types, enums, constants      | `types/Taskschd.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/taskschd` directly.

## Calling Convention

All documented `taskschd.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Taskschd.SomeFunctionW(wide.ptr);
