# AI Guide for @bun-win32/wlanapi
...
```ts
import Wlanapi, { SomeFlag } from '@bun-win32/wlanapi';

// Methods bind lazily on first call
const result = Wlanapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wlanapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Wlanapi.Preload('SomeFunctionW');
Wlanapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Wlanapi.ts` |
| Find types, enums, constants      | `types/Wlanapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types - import from `@bun-win32/wlanapi` directly.

## Calling Convention

All documented `wlanapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Wlanapi.SomeFunctionW(wide.ptr);
