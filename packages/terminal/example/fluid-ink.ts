...
const INK_INDIGO: RGB = [0.1, 0.3, 1.15];
const INK_MAGENTA: RGB = [1.2, 0.16, 0.78];
const INK_AMBER: RGB = [1.3, 0.62, 0.12];
const inkColor = (tint: number): RGB => {
  // 0 → indigo, 0.5 → magenta, 1 → amber
  if (tint < 0.5) {
    const k = tint * 2;
    return [lerp(INK_INDIGO[0], INK_MAGENTA[0], k), lerp(INK_INDIGO[1], INK_MAGENTA[1], k), lerp(INK_INDIGO[2], INK_MAGENTA[2], k)];
  }
  const k = (tint - 0.5) * 2;
  return [lerp(INK_MAGENTA[0], INK_AMBER[0], k), lerp(INK_MAGENTA[1], INK_AMBER[1], k), lerp(INK_MAGENTA[2], INK_AMBER[2], k)];
};
...
interface Emitter {
  ...
  tintDrift: number;
  cx: number;
  cy: number; // path centre (fraction of grid) — spread across the tank
  ax: number;
  ay: number;
  fx: number;
  fy: number;
...
const buildEmitters = (): void => {
  ...
  const centresX = [0.12, 0.3, 0.5, 0.7, 0.88, 0.42];
  for (let k = 0; k < anchors.length; k++) {
    emitters.push({
      tint: anchors[k],
      tintDrift: (rng() - 0.5) * 0.05,
      cx: centresX[k],
      cy: 0.6 + rng() * 0.22, // lower band — plumes rise through the frame
...
const step = (time: number, dt: number): void => {
  ...
  diffuse(dr, s0, DYE_DIFFUSE);
  diffuse(dg, s1, DYE_DIFFUSE);
  diffuse(db, s2, DYE_DIFFUSE);
  setScalarBounds(dr);
  setScalarBounds(dg);
  setScalarBounds(db);
};
...
const stroke = (x0: number, y0: number, dx: number, dy: number, bend: number, rad: number, tint: number, amt: number): void => {
  const [cr, cg, cb] = inkColor(tint);
  const steps = 22;
  ...
  for (let s = 0; s <= steps; s++) {
    ...
    splat(px, py, rad * (0.6 + 0.4 * taper), dx * 0.012, dy * 0.012 - 0.4, cr, cg, cb, amt * taper);
  }
};
const seed = (): void => {
  u.fill(0);
  ...
  const left = GW * 0.16 + 1,
    mid = GW * 0.5 + 1,
    right = GW * 0.84 + 1;
  stroke(left, cy + 10, GW * 0.3, -16, 11, 2.2, 0.03, 0.3); // indigo, low-left → up-right
  stroke(mid - 10, cy + 14, GW * 0.16, -26, -13, 2.0, 0.5, 0.32); // magenta, sweeping up through centre
  stroke(right, cy + 8, -GW * 0.3, -14, -11, 2.2, 0.96, 0.3); // amber, low-right → up-left (crosses the indigo)
  stroke(left * 0.6, cy - 6, GW * 0.12, -8, 7, 1.7, 0.2, 0.24); // indigo/violet edge accent
  stroke(right + 14, cy - 2, -GW * 0.1, -9, -7, 1.7, 0.78, 0.24); // warm edge accent
