...
const DISK_B = new Float32Array(LUT_N);
{
  ...
  for (let i = 0; i < LUT_N; i++) {
    ...
    if (x < 0.18) {
      ...
      r = lerp(1.0, 1.0, k);
      g = lerp(0.97, 0.86, k);
      b = lerp(0.92, 0.58, k); // blue-white → warm white
    } else if (x < 0.5) {
      const k = (x - 0.18) / 0.32;
      r = lerp(1.0, 1.0, k);
      g = lerp(0.86, 0.62, k);
      b = lerp(0.58, 0.22, k); // warm white → amber
    } else {
      const k = (x - 0.5) / 0.5;
      r = lerp(1.0, 0.74, k);
      g = lerp(0.62, 0.2, k);
      b = lerp(0.22, 0.06, k); // amber → ember
    }
    DISK_R[i] = r;
    DISK_G[i] = g;
    DISK_B[i] = b;
...
const vnoise = (x: number, y: number, z: number): number => {
  ...
  const x0 = Math.imul(xi, 374761393),
    x1 = Math.imul(xi + 1, 374761393);
  const y0h = Math.imul(yi, 668265263),
    y1h = Math.imul(yi + 1, 668265263);
  const z0h = Math.imul(zi, 2147483647),
    z1h = Math.imul(zi + 1, 2147483647);
  const c000 = vmix(x0 ^ y0h ^ z0h),
    c100 = vmix(x1 ^ y0h ^ z0h);
  const c010 = vmix(x0 ^ y1h ^ z0h),
    c110 = vmix(x1 ^ y1h ^ z0h);
  const c001 = vmix(x0 ^ y0h ^ z1h),
    c101 = vmix(x1 ^ y0h ^ z1h);
  const c011 = vmix(x0 ^ y1h ^ z1h),
    c111 = vmix(x1 ^ y1h ^ z1h);
  const xa = c000 + (c100 - c000) * fx,
    xb = c010 + (c110 - c010) * fx;
  const xc = c001 + (c101 - c001) * fx,
    xd = c011 + (c111 - c011) * fx;
  const ya = xa + (xb - xa) * fy,
    yb = xc + (xd - xc) * fy;
  return ya + (yb - ya) * fz;
};
...
const initBuffers = (t: Term): void => {
  ...
  upW = 0;
  upRW = 0; // force bilinear column LUT rebuild
};
...
const sampleDisk = (r: number, phi: number, doppler: number, time: number): void => {
  ...
  const inner = smoothstep(0.1, 0.0, t);
  ...
  cr = lerp(cr * 1.3, cr * 0.74, blu);
  cg = lerp(cg * 0.92, cg * 1.04, blu);
  cb = lerp(cb * 0.5, cb * 1.55, blu);
...
let camRoX = 0,
  camRoY = 0,
  camRoZ = 0; // ray origin (camera position)
let fwdX = 0,
  fwdY = 0,
  fwdZ = 0; // forward
let rgtX = 0,
...
let dax = 0,
  day = 0,
  daz = 0; // in-plane axis A (phi=0 direction)
let dbx = 0,
  dby = 0,
  dbz = 0; // in-plane axis B
...
const DIST_MIN = 13.0,
  DIST_MAX = 45.0; // keep the camera outside the disk, never uselessly far
...
const ZOOM_KEY = 0.9; // dist *= this per +/− key (zoom in)

let camAzim = AZIM0,
  camElev = ELEV0; // live camera angles (manual mode)
let camDist = DIST0,
  tgtDist = DIST0; // live distance eased toward the zoom target
...
let prevMX = 0,
  prevMY = 0; // previous mouse pixel pos (for the drag delta)
...
let kbAzim = 0,
  kbElev = 0; // pending arrow-key nudges
...
const acc3 = [0, 0, 0];
let hitT = 0;
const sampleDiskAlongRay = (ox: number, oy: number, oz: number, dirx: number, diry: number, dirz: number, time: number): boolean => {
...
const shadePixel = (ix: number, iy: number, time: number): void => {
  ...
  const cx = chx,
    cy = chy,
    cz = chz; // camera → hole vector (hoisted)
  const along = cx * dx + cy * dy + cz * dz;
  const perpx = cx - along * dx,
    perpy = cy - along * dy,
    perpz = cz - along * dz;
  ...
  if (!captured && along > 0) {
    ...
    if (ring > 0.002) {
      ...
      const px = -bx,
        py = -by,
        pz = -bz; // outward radial in the bend plane
...
const frame = (t: Term, time: number, dt: number, _frameNo: number): void => {
  ...
  const keyAzim = kbAzim,
    keyElev = kbElev,
    keyZoom = kbZoom,
    doReset = pendingReset;
  kbAzim = 0;
  kbElev = 0;
  kbZoom = 0;
  pendingReset = false;
  const wheel = t.mouse.wheel;
  t.mouse.wheel = 0;
  ...
  if (down && !dragging) {
    ...
    prevMY = t.mouse.y;
    velAzim = 0;
  } // press: anchor, no jump, fresh flick
  ...
  chx = -camRoX;
  chy = -camRoY;
  chz = -camRoZ; // camera → hole, hoisted for shadePixel
  ...
  dax = 1;
  day = 0;
  daz = 0; // in-plane axis A (phi = 0)
  dbx = 0;
  dby = 0;
  dbz = 1; // in-plane axis B (phi = 90°)
  ...
  if (upW !== W || upRW !== RW) {
    if (!colI0 || colI0.length !== W) {
      colI0 = new Int32Array(W);
      colI1 = new Int32Array(W);
      colWx = new Float32Array(W);
      colVx = new Float32Array(W);
    }
    for (let x = 0; x < W; x++) {
      let fxv = (x + 0.5) * sxs - 0.5;
      if (fxv < 0) fxv = 0;
      else if (fxv > RW - 1) fxv = RW - 1;
      const x0 = fxv | 0,
        x1 = x0 + 1 < RW ? x0 + 1 : x0;
      colI0[x] = x0 * 3;
      colI1[x] = x1 * 3;
