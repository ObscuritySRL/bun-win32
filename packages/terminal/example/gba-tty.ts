...
function setupAnsiInput(): InputSource {
  ...
  const onData = (data: Buffer): void => {
    ...
    for (let i = 0; i < s.length; i += 1) {
      ...
      else if (up === 'A') press(VK.A);
      else if (up === 'S') press(VK.S);
...
function readButtons(src: InputSource): GbaButtons {
  ...
  if (Xinput1_4.XInputGetState(0, xinputBuf.ptr!) === 0) {
    ...
    const lx = xinputBuf.readInt16LE(8),
      ly = xinputBuf.readInt16LE(10),
      DZ = 7849;
    if (btn & 0x1000) a = true;
    if (btn & 0x2000) b = true;
    if (btn & 0x0010) start = true;
    if (btn & 0x0020) select = true;
    if (btn & 0x0100) l = true;
    if (btn & 0x0200) r = true;
...
function blitToTerm(t: Term, frame: Uint8Array): void {
  ...
  const dw = Math.max(1, Math.floor(GBA_W * scale)),
    dh = Math.max(1, Math.floor(GBA_H * scale));
  const ox = ((W - dw) / 2) | 0,
    oy = ((H - dh) / 2) | 0;
  ...
  for (let y = 0; y < dh; y += 1) {
    let gy = (y * inv) | 0;
    if (gy >= GBA_H) gy = GBA_H - 1;
    const rowG = gy * GBA_W,
      rowT = oy + y;
    for (let x = 0; x < dw; x += 1) {
      let gx = (x * inv) | 0;
      if (gx >= GBA_W) gx = GBA_W - 1;
      const o = (rowG + gx) * 4;
...
async function main(): Promise<void> {
  ...
  const feedAudio = (): void => {
    ...
    if (pcm.available && !muted) {
      if (pcm.queued() < AUDIO_TARGET) {
        const b = gba.apu.drain(AUDIO_RATE);
        if (b.length) pcm.submit(b);
      }
    } else {
      gba.apu.drain(AUDIO_RATE);
    }
  };
  if (pcm.available) {
    pcm.setVolume(0.6);
    for (let i = 0; i < AUDIO_TARGET; i += 1) {
      gba.runFrame();
      const b = gba.apu.drain(AUDIO_RATE);
      if (b.length) pcm.submit(b);
    }
  ...
  if (existsSync(savePath)) {
    try {
      ...
      gba.loadSaveData(new Uint8Array(f.buffer, f.byteOffset, f.byteLength));
    } catch {
      /* ignore */
    }
  }
  ...
  const autoSave = (): void => {
    if (saving) return;
    saving = true;
    Bun.write(savePath, gba.getSaveData())
      .catch(() => {
        /* ignore */
      })
      .finally(() => {
  ...
  const writeSaveSync = (): void => {
    try {
      writeFileSync(savePath, gba.getSaveData());
    } catch {
      /* ignore */
    }
  };
  ...
  try {
    while (running) {
      ...
      if (elapsedTotal - lastSaveMs >= 5000) {
        lastSaveMs = elapsedTotal;
        autoSave();
      }
      ...
      for (const vk of input.pressed) {
        if (vk === VK.P) paused = !paused;
        else if (vk === VK.M) muted = !muted;
      }
      ...
      if (turbo) {
        ...
        if (wait) wait(FRAME_MS);
        else await Bun.sleep(FRAME_MS);
      } else {
        const slack = Math.min(FRAME_MS, nextDue - Bun.nanoseconds() / 1e6);
        if (slack > 0.2) {
          if (wait) wait(slack);
