...
const PITCH = envN('PITCH', 0.4); // arm pitch angle (rad); ~23° → open arms
...
const ARM_WIDTH = envN('ARMW', 0.4); // angular half-width of an arm crest (rad)
const ARM_R0 = GALAXY_R * 0.1; // inside this, arms dissolve into the bulge
...
const initSim = (): void => {
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    const thick = isBulge ? BULGE_R * 0.55 * Math.pow(1 - Math.min(1, r / BULGE_R), 0.5) : 0.035 * GALAXY_R * (0.5 + 0.7 * Math.min(1, rNorm));
    yoff[i] = gauss() * thick;
    ...
    let tc: number;
    if (isBulge) tc = 0.4 + gauss() * 0.05;
    else tc = 0.46 - 0.26 * rNorm + gauss() * 0.1;
...
const TEMP_LUT_B = new Float32Array(256);
{
  for (let i = 0; i < 256; i++) {
    ...
    if (tc > 0.62) {
      ...
      r = lerp(0.74, 0.58, k);
      g = lerp(0.84, 0.74, k);
      b = lerp(1.0, 1.0, k); // blue-white → blue
    } else if (tc > 0.42) {
      const k = (tc - 0.42) / 0.2;
      r = lerp(1.0, 0.74, k);
      g = lerp(0.9, 0.84, k);
      b = lerp(0.68, 1.0, k); // white → blue-white
    } else if (tc > 0.22) {
      const k = (tc - 0.22) / 0.2;
      r = lerp(1.0, 1.0, k);
      g = lerp(0.6, 0.9, k);
      b = lerp(0.28, 0.68, k); // gold → white
    } else {
      const k = tc / 0.22;
      r = lerp(0.93, 1.0, k);
      g = lerp(0.32, 0.6, k);
      b = lerp(0.2, 0.28, k); // red → gold
    }
    TEMP_LUT_R[i] = r;
    TEMP_LUT_G[i] = g;
    TEMP_LUT_B[i] = b;
...
const allocForSize = (W: number, H: number, aspect: number): void => {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const g3 = g * g * g;
      starBuf[o] = 0.0016 + 0.003 * g3;
      starBuf[o + 1] = 0.0024 + 0.005 * g3;
...
const frame = (t: Term, time: number, _dt: number, _frameNo: number): void => {
  ...
  densNorm = densNorm < 1 ? 0.3 + 0.7 * densNorm * densNorm : 1;
  const cosI = Math.cos(1.02),
    sinI = Math.sin(1.02); // inclination
  ...
  const e0a = ARM_R0 / GALAXY_R,
    invSpanA = 1 / (0.26 - e0a);
  const e0b = 1.32,
    invSpanB = 1 / (1.05 - 1.32);
  ...
  const CORE_TAPER_R = 0.22,
    INV_CORE_TAPER_R = 1 / CORE_TAPER_R,
    CORE_TAPER_MIN = 0.1;
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    const rn = r * INV_GR;
    let fA = (rn - e0a) * invSpanA;
    fA = fA < 0 ? 0 : fA > 1 ? 1 : fA;
    fA = fA * fA * (3 - 2 * fA);
    let fB = (rn - e0b) * invSpanB;
    fB = fB < 0 ? 0 : fB > 1 ? 1 : fB;
    fB = fB * fB * (3 - 2 * fB);
    armW *= fA * fB;
    ...
    let sh = (depth + GALAXY_R) * INV_2GR;
    sh = sh < 0 ? 0 : sh > 1 ? 1 : sh;
    const shade = 0.8 + 0.2 * sh;
    ...
    let cTap = densNorm;
    if (rn < CORE_TAPER_R) {
      const u = rn * INV_CORE_TAPER_R;
      cTap *= CORE_TAPER_MIN + (1 - CORE_TAPER_MIN) * (u * u * (3 - 2 * u));
    }
  ...
  // glow. Kept restrained (low blue, sub-clip peak) so bloom can't blow it out to grey.
  {
    ...
    const y0 = Math.max(1, (cy - coreR) | 0),
      y1 = Math.min(H - 2, (cy + coreR) | 0);
    const x0 = Math.max(1, (cx - coreR * aspect) | 0),
      x1 = Math.min(W - 2, (cx + coreR * aspect) | 0);
  ...
  {
    const thr = 0.3; // higher knee → only true highlights bloom; arms/core stay crisp
    for (let y = 0; y < bh; y++) {
      const sy0 = y * 2;
      for (let x = 0; x < bw; x++) {
        const sx0 = x * 2;
  ...
  for (let yy = 0; yy < H; yy++) {
    ...
    const wzpC = wzp * cosV,
      wzpS = wzp * sinV; // partials for un-rotation
    const bRow = (yy >> 1) * bw;
