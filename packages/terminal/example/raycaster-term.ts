...
const buildMaze = (): void => {
  ...
  const carveRoom = (rx: number, ry: number, rw: number, rh: number): void => {
    for (let y = ry; y < ry + rh; y++) for (let x = rx; x < rx + rw; x++) if (x > 0 && y > 0 && x < MAP_W - 1 && y < MAP_H - 1) grid[y * MAP_W + x] = 0;
  };
  carveRoom(3, 3, 5, 4);
  carveRoom(15, 5, 5, 5);
  carveRoom(6, 16, 6, 5);
  ...
  for (let y = 0; y < MAP_H; y++) {
    for (let x = 0; x < MAP_W; x++) {
      ...
      const nearOpen = (cell(x + 1, y) === 0 && cell(x + 2, y) === 0) || (cell(x - 1, y) === 0 && cell(x - 2, y) === 0) || (cell(x, y + 1) === 0 && cell(x, y + 2) === 0) || (cell(x, y - 1) === 0 && cell(x, y - 2) === 0);
...
const wallTexel = (mat: number, u: number, v: number): number => {
  ...
  if (mat === 1 || mat === 4) {
    ...
    const round = smoothstep(0.0, 0.25, inU) * smoothstep(1.0, 0.75, inU) * smoothstep(0.0, 0.3, inV) * smoothstep(1.0, 0.7, inV);
    return 0.6 + jit * 0.3 + round * 0.22;
  }
  if (mat === 2) {
    ...
    if (inU < 0.08 || inU > 0.92 || inV < 0.1 || inV > 0.9) return 0.28 + moss * 0.1;
  ...
  if (inU < 0.05 || inV < 0.06) return 0.4; // seam
  const bevel = smoothstep(0.0, 0.16, inU) * smoothstep(1.0, 0.84, inU) * smoothstep(0.0, 0.18, inV) * smoothstep(1.0, 0.82, inV);
...
const hold = (k: keyof typeof keys): void => {
  keys[k] = true;
  releaseTimers[k] = simTime.v + 0.16;
};
...
const frame = (t: Term, time: number, dt: number, _frame: number): void => {
  ...
  const probe = (a: number): number => {
    ...
    while (d < 8) {
      d += 0.25;
      if (isWall(player.x + cx * d, player.y + sy * d)) break;
    }
    return d;
  };
  ...
  if (attract) {
    ...
    const ca = Math.cos(player.ang),
      sa = Math.sin(player.ang);
  ...
  const dirX = Math.cos(player.ang),
  ...
  const rayDir0X = dirX - planeX,
    rayDir0Y = dirY - planeY; // cameraX = -1
  const rayDir1X = dirX + planeX,
    rayDir1Y = dirY + planeY; // cameraX = +1
  ...
  const order = sprites.map((s, i) => ({ i, d: (s.x - posX) * (s.x - posX) + (s.y - posY) * (s.y - posY) })).sort((a, b) => b.d - a.d); // far → near
  ...
  for (const { i } of order) {
    ...
    const yTop = (cys - rad) | 0,
      yBot = (cys + rad) | 0;
    const xL = (screenX - rad) | 0,
      xR = (screenX + rad) | 0;
...
const drawMinimap = (t: Term): void => {
  ...
  const mw = MAP_W * px,
    mh = MAP_H * px;
  const ox = W - mw - 2,
    oy = 2; // top-right corner, 2px margin
  ...
  for (let cy = 0; cy < MAP_H; cy++) {
    for (let cx = 0; cx < MAP_W; cx++) {
      ...
      if (c === 0) {
        ...
        const base = MAT_BASE[c] ?? MAT_BASE[1];
        r = (base[0] * 0.55) | 0;
        g = (base[1] * 0.55) | 0;
        b = (base[2] * 0.55) | 0;
      }
