...
{
  const base = [
    0, 32, 8, 40, 2, 34, 10, 42, 48, 16, 56, 24, 50, 18, 58, 26, 12, 44, 4, 36, 14, 46, 6, 38, 60, 28, 52, 20, 62, 30, 54, 22, 3, 35, 11, 43, 1, 33, 9, 41, 51, 19, 59, 27, 49, 17, 57, 25, 15, 47, 7, 39, 13, 45, 5, 37, 63, 31, 55, 23, 61,
    29, 53, 21,
  ];
...
const SUN_DIR_X = -0.62,
  SUN_DIR_Y = 0.34,
  SUN_DIR_Z = -0.7; // toward the sun
const SUN_LEN = Math.hypot(SUN_DIR_X, SUN_DIR_Y, SUN_DIR_Z);
const SLX = SUN_DIR_X / SUN_LEN,
  SLY = SUN_DIR_Y / SUN_LEN,
  SLZ = SUN_DIR_Z / SUN_LEN;
...
const vnoise2 = (x: number, y: number, period: number): number => {
  ...
  const hx0 = Math.imul(x0, 374761393),
    hx1 = Math.imul(x1, 374761393);
  const hy0 = Math.imul(y0, 668265263),
    hy1 = Math.imul(y1, 668265263);
  const c00 = vmix(hx0 ^ hy0),
    c10 = vmix(hx1 ^ hy0);
  const c01 = vmix(hx0 ^ hy1),
    c11 = vmix(hx1 ^ hy1);
...
const sampleAlbedo = (h: number, jitter: number): void => {
  ...
  if (h < 0.3) {
    ...
    const k = h / 0.3;
    r = lerp(0.02, 0.58, smoothstep(0.6, 1.0, k));
    ...
    const k = (h - 0.3) / 0.22; // golden sand → lush saturated grass (a touch deeper)
    r = lerp(0.87, 0.15, k);
    g = lerp(0.74, 0.55, k);
    b = lerp(0.47, 0.11, k);
  } else if (h < 0.72) {
    const k = (h - 0.52) / 0.2; // grass → warm rock (deeper green dips to ochre rock)
    r = lerp(0.15, 0.53, k);
    g = lerp(0.55, 0.43, k);
    b = lerp(0.11, 0.31, k);
  } else if (h < 0.86) {
...
const buildMap = (): void => {
  ...
  // so coastlines stay sharp. Three light passes for a soft, rounded relief.
  {
    ...
    const w0 = 0.34,
      w1 = 0.24,
      w2 = 0.09; // normalized 5-tap (0.34+2*0.24+2*0.09=1)
    for (let pass = 0; pass < 5; pass++) {
      ...
      for (let y = 0; y < MAP_N; y++) {
        ...
        for (let x = 0; x < MAP_N; x++) {
          ...
          if (c < 0.3) {
            tmp[r + x] = c;
            continue;
          } // keep shorelines/seas crisp
          const xm2 = (x - 2) & MAP_MASK,
            xm1 = (x - 1) & MAP_MASK;
          const xp1 = (x + 1) & MAP_MASK,
            xp2 = (x + 2) & MAP_MASK;
      ...
      for (let y = 0; y < MAP_N; y++) {
        const r = y * MAP_N;
        const ym2 = ((y - 2) & MAP_MASK) * MAP_N,
          ym1 = ((y - 1) & MAP_MASK) * MAP_N;
        const yp1 = ((y + 1) & MAP_MASK) * MAP_N,
          yp2 = ((y + 2) & MAP_MASK) * MAP_N;
        for (let x = 0; x < MAP_N; x++) {
          const c = tmp[r + x];
          if (c < 0.3) {
            HMAP[r + x] = c;
  ...
  for (let y = 0; y < MAP_N; y++) {
    ...
    for (let x = 0; x < MAP_N; x++) {
      ...
      const cloudShadow = lerp(1.0, 0.62, smoothstep(0.5, 0.8, cloud));
      ...
      if (h < 0.3) {
        const w = smoothstep(0.3, 0.0, h);
        r = lerp(r, r * 0.66 + 0.05, w);
        g = lerp(g, g * 0.66 + 0.1, w);
  ...
  // ranges read as soft massifs, not a comb of pickets. Land only — seas stay flat.
  {
    ...
    const a0 = 0.24,
      a1 = 0.2,
      a2 = 0.12,
      a3 = 0.06; // 7-tap (0.24+2*.20+2*.12+2*.06=1)
    for (let pass = 0; pass < 4; pass++) {
      for (let y = 0; y < MAP_N; y++) {
        const r = y * MAP_N;
        for (let x = 0; x < MAP_N; x++) {
          const xm3 = (x - 3) & MAP_MASK,
            xm2 = (x - 2) & MAP_MASK,
            xm1 = (x - 1) & MAP_MASK;
          const xp1 = (x + 1) & MAP_MASK,
            xp2 = (x + 2) & MAP_MASK,
            xp3 = (x + 3) & MAP_MASK;
          tmp[r + x] = HMAP_FAR[r + x] * a0 + (HMAP_FAR[r + xm1] + HMAP_FAR[r + xp1]) * a1 + (HMAP_FAR[r + xm2] + HMAP_FAR[r + xp2]) * a2 + (HMAP_FAR[r + xm3] + HMAP_FAR[r + xp3]) * a3;
        }
      }
      for (let y = 0; y < MAP_N; y++) {
        const r = y * MAP_N;
        const ym3 = ((y - 3) & MAP_MASK) * MAP_N,
          ym2 = ((y - 2) & MAP_MASK) * MAP_N,
          ym1 = ((y - 1) & MAP_MASK) * MAP_N;
        const yp1 = ((y + 1) & MAP_MASK) * MAP_N,
          yp2 = ((y + 2) & MAP_MASK) * MAP_N,
          yp3 = ((y + 3) & MAP_MASK) * MAP_N;
        for (let x = 0; x < MAP_N; x++) {
          HMAP_FAR[r + x] = tmp[r + x] * a0 + (tmp[ym1 + x] + tmp[yp1 + x]) * a1 + (tmp[ym2 + x] + tmp[yp2 + x]) * a2 + (tmp[ym3 + x] + tmp[yp3 + x]) * a3;
        }
      }
    }
  }
...
const buildSky = (H: number): void => {
  ...
  skyH = H;
  SKY_R = new Float32Array(H);
  SKY_G = new Float32Array(H);
  SKY_B = new Float32Array(H);
  for (let y = 0; y < H; y++) {
    ...
    let r = lerp(0.035, 0.3, tg);
    let g = lerp(0.065, 0.37, tg);
    let b = lerp(0.27, 0.5, tg);
    ...
    const violet = Math.exp(-((t - 0.4) * (t - 0.4)) * 24);
    r += violet * 0.115;
    g += violet * 0.03;
    b += violet * 0.075;
...
const frame = (t: Term, time: number, dt: number, _frameNo: number): void => {
  ...
  if (rowSrcW !== RW) {
    rowSrc = new Uint8Array(RW * 3);
    rowSrcW = RW;
  }
  if (sbufLen !== RW * H * 3) {
    sbuf = new Uint8Array(RW * H * 3);
    sbufLen = RW * H * 3;
  }
  ...
  if (t.mouse.active && t.mouse.sequence !== lastMouseSeq) {
    if (lastMouseX >= 0) {
      ...
      if (Math.abs(dxm) > 0 || Math.abs(dym) > 0) {
        userControlled = true;
        lastInputT = time;
      }
    }
    lastMouseX = t.mouse.x;
    lastMouseY = t.mouse.y;
  ...
  if (attract) {
    ...
    const turn = 0.3 * Math.sin(time * 0.13) + 0.14 * Math.sin(time * 0.37 + 1.1);
  ...
  {
    const hx = Math.cos(cam.yaw),
      hz = Math.sin(cam.yaw);
    let maxGround = 0;
    for (let s = 0; s <= 4; s++) {
      const look = s * 70; // 0,70,…,280 world units ahead
      const gx = cam.x + hx * look,
        gz = cam.z + hz * look;
      const gxf = Math.floor(gx),
        gzf = Math.floor(gz);
      const fx = gx - gxf,
        fz = gz - gzf;
      const ax = gxf & MAP_MASK,
        az = gzf & MAP_MASK;
      const bx = (ax + 1) & MAP_MASK,
        bz = (az + 1) & MAP_MASK;
      const r0 = az * MAP_N,
        r1 = bz * MAP_N;
      const g00 = HMAP[r0 + ax],
        g10 = HMAP[r0 + bx];
      const g01 = HMAP[r1 + ax],
        g11 = HMAP[r1 + bx];
      const ga = g00 + (g10 - g00) * fx,
        gb = g01 + (g11 - g01) * fx;
  ...
  while (sunBearing < -Math.PI) sunBearing += 2 * Math.PI;
  const sunOnScreen = Math.abs(sunBearing) < halfFov + 0.3;
  ...
  for (let x = 0; x < RW; x++) {
    ...
    const pillar = sunOnScreen ? Math.exp(-dxs2 * 0.01) : 0;
    let o = x * 3;
    ...
    for (let y = 0; y < H; y++) {
      ...
      if (sunOnScreen) {
        ...
        const core = Math.exp(-d2 * 0.012);
        r += core * 1.7;
        g += core * 1.42;
        b += core * 0.86;
        // sun pillar: a soft vertical shaft of warm light through the disk
        const shaft = pillar * Math.exp(-dys2 * 0.0005);
        r += shaft * 0.4;
        g += shaft * 0.26;
        b += shaft * 0.1;
  ...
  const leftA = cam.yaw - halfFov,
    rightA = cam.yaw + halfFov;
  const dirLX = Math.cos(leftA),
    dirLZ = Math.sin(leftA);
  const dirRX = Math.cos(rightA),
    dirRZ = Math.sin(rightA);
  ...
  const HAZE_COOL_R = FOG_LR,
    HAZE_COOL_G = FOG_LG,
    HAZE_COOL_B = FOG_LB;
  const HAZE_WARM_R = 0.92,
    HAZE_WARM_G = 0.62,
    HAZE_WARM_B = 0.4;
  ...
  const cloudPhX = time * 7.0,
    cloudPhZ = time * 4.0; // shadows drift downwind
  const CLOUD_S1 = 0.0042,
    CLOUD_S2 = 0.0027; // two spatial scales
  ...
  for (let x = 0; x < RW; x++) {
    ...
    while (z < Z_FAR && ytop > 0) {
      ...
      const wxf = Math.floor(wx),
        wzf = Math.floor(wz);
      const fx = wx - wxf,
        fz = wz - wzf;
      const ix0 = wxf & MAP_MASK,
        iz0 = wzf & MAP_MASK;
      const ix1 = (ix0 + 1) & MAP_MASK,
        iz1 = (iz0 + 1) & MAP_MASK;
      const r0 = iz0 * MAP_N,
        r1 = iz1 * MAP_N;
      const h00 = HMAP[r0 + ix0],
        h10 = HMAP[r0 + ix1];
      const h01 = HMAP[r1 + ix0],
        h11 = HMAP[r1 + ix1];
      const ha = h00 + (h10 - h00) * fx,
        hb = h01 + (h11 - h01) * fx;
      ...
      if (z > LOD_NEAR) {
        const f00 = HMAP_FAR[r0 + ix0],
          f10 = HMAP_FAR[r0 + ix1];
        const f01 = HMAP_FAR[r1 + ix0],
          f11 = HMAP_FAR[r1 + ix1];
        const fa = f00 + (f10 - f00) * fx,
          fb = f01 + (f11 - f01) * fx;
      ...
      if (projY < ytop) {
        ...
        if (fogA < 0.62) {
          if (hn >= 0.3) {
            ...
            const cf = Math.sin((wx + cloudPhX) * CLOUD_S1) * Math.sin((wz - cloudPhZ) * CLOUD_S1) + 0.6 * Math.sin((wx - cloudPhZ) * CLOUD_S2 + 1.7) * Math.sin((wz + cloudPhX) * CLOUD_S2 - 0.9);
            ...
            const calm = smoothstep(0.3, 0.2, hn); // glints on open water, not the sand lip
            const sw = Math.sin(wx * 0.09 + wz * 0.052 + glitPh) * Math.sin(wz * 0.084 - wx * 0.041 - glitPh2);
            let spark = sw * 0.5 + 0.5; // 0..1
            spark *= spark;
            spark *= spark; // ^4 → thin bright crests
            const gl = spark * (0.3 + 0.7 * sunw) * calm * (1 - fogA);
            cr = (cr + gl * 150) | 0;
            cg = (cg + gl * 116) | 0;
            cb = (cb + gl * 66) | 0;
            if (cr > 255) cr = 255;
      ...
      z += dz;
      dz += 0.02 * dz; // exponential step growth keeps far cost flat
    }
    ybuf[x] = ytop;
  }
  ...
  // Reads from a per-row source snapshot so the blur never feeds back on itself.
  {
    ...
    const W0 = 0.4,
      W1 = 0.22,
      W2 = 0.08; // 0.40 + 2*0.22 + 2*0.08 = 1.0
