...
const REGIMES: [number, number][] = [
  [0.054, 0.0615], // moving spots (κ)
  [0.058, 0.0608], // mitosis — dividing cells
  [0.04, 0.06], // dense worms
  [0.037, 0.061], // worms + loops (θ)
  [0.03, 0.056], // coral / branching growth
  [0.034, 0.057], // coral, denser
  [0.039, 0.058], // mazes / labyrinth (λ)
  [0.046, 0.0655], // pulsing holes / chaos (μ)
  [0.025, 0.059], // unstable worms / mitosis-like division
];
...
const seed = (): void => {
  ...
  for (let b = 0; b < blobs; b++) {
    ...
    const x0 = Math.max(0, (cx - r) | 0),
      x1 = Math.min(GW - 1, (cx + r) | 0);
    const y0 = Math.max(0, (cy - r) | 0),
      y1 = Math.min(GH - 1, (cy + r) | 0);
    for (let y = y0; y <= y1; y++) {
      const dy = y - cy;
      for (let x = x0; x <= x1; x++) {
        const dx = x - cx;
...
const buildFK = (time: number): void => {
  ...
  const dca = Math.cos(dAng),
    dsa = Math.sin(dAng);
  const dox = time * 0.008,
    doy = -time * 0.011;
  ...
  for (let y = 0; y < GH; y++) {
    ...
    for (let x = 0; x < GW; x++) {
      ...
      sel = sel * 0.7 + sampleLat(latB, rx * f2 - oy * 1.3, ry * f2 + ox * 1.1) * 0.3;
...
const stepCell = (
  ...
  const uc = u[i];
  const vc = v[i];
  const lu = (u[il] + u[ir] + u[iu] + u[id]) * 0.2 + (u[iul] + u[iur] + u[idl] + u[idr]) * 0.05 - uc;
  const lv = (v[il] + v[ir] + v[iu] + v[id]) * 0.2 + (v[iul] + v[iur] + v[idl] + v[idr]) * 0.05 - vc;
  const F = f[i],
    K = k[i];
  const uvv = uc * vc * vc;
...
const step = (): void => {
  ...
  for (let y = 1; y < GH - 1; y++) {
    ...
    for (let x = 1; x < GW - 1; x++) {
      ...
      const iu = ym + x,
        id = yp + x;
      const lu = (u[i - 1] + u[i + 1] + u[iu] + u[id]) * 0.2 + (u[iu - 1] + u[iu + 1] + u[id - 1] + u[id + 1]) * 0.05 - uc;
      const lv = (v[i - 1] + v[i + 1] + v[iu] + v[id]) * 0.2 + (v[iu - 1] + v[iu + 1] + v[id - 1] + v[id + 1]) * 0.05 - vc;
      const F = f[i],
        K = k[i];
      const uvv = uc * vc * vc;
      const nu = uc + DU * lu - uvv + F * (1 - uc);
...
const RAMP: [number, number, number][] = [
  ...
  [0.13, 0.49, 0.476], // bright teal
  [0.47, 0.72, 0.56], // jade, warmer
  [0.91, 0.87, 0.66], // warm bone — amber-biased crest
  [1.0, 0.965, 0.84], // hot bone-white, warm
];
...
let xCx0 = new Int32Array(0),
  xCx1 = new Int32Array(0),
  xCxm = new Int32Array(0),
  xCxp = new Int32Array(0);
let xFx = new Float32Array(0);
let lastSampW = -1;
const buildXTable = (W: number): void => {
  if (xCx0.length !== W) {
    xCx0 = new Int32Array(W);
    xCx1 = new Int32Array(W);
    xCxm = new Int32Array(W);
    xCxp = new Int32Array(W);
...
const render = (t: Term, time: number): void => {
  ...
  for (let py = 0; py < H; py++) {
    ...
    for (let px = 0; px < W; px++) {
      ...
      const v00 = v[row0 + cx0],
        v10 = v[row0 + cx1];
      const v01 = v[row1 + cx0],
        v11 = v[row1 + cx1];
      ...
      const shade = (0.3 + 0.78 * lam) * (0.55 + 0.45 * cav);
      r *= shade;
      g *= shade;
      bl *= shade;
      ...
      let sw = (val - 0.1) * 5; // 1/(0.30-0.10) = 5
      sw = sw < 0 ? 0 : sw > 1 ? 1 : sw;
...
let bvW = -1,
  bvH = -1;
let bXm1 = new Int32Array(0),
  bXp1 = new Int32Array(0),
  bXm2 = new Int32Array(0),
  bXp2 = new Int32Array(0);
const buildBloomTables = (W: number, H: number): void => {
  ...
  const r1 = 2,
    r2 = 5;
  bXm1 = new Int32Array(W);
  bXp1 = new Int32Array(W);
  bXm2 = new Int32Array(W);
  bXp2 = new Int32Array(W);
  for (let x = 0; x < W; x++) {
    bXm1[x] = x - r1 < 0 ? 0 : x - r1;
    bXp1[x] = x + r1 >= W ? W - 1 : x + r1;
    bXm2[x] = x - r2 < 0 ? 0 : x - r2;
    bXp2[x] = x + r2 >= W ? W - 1 : x + r2;
  }
};
...
const bloomVignette = (t: Term, time: number): void => {
  ...
  const xm1a = bXm1,
    ...
    xp2a = bXp2;
  for (let y = 0; y < H; y++) {
    const ym1 = (y - r1 < 0 ? 0 : y - r1) * W,
      yp1 = (y + r1 >= H ? H - 1 : y + r1) * W;
    const ym2 = (y - r2 < 0 ? 0 : y - r2) * W,
      yp2 = (y + r2 >= H ? H - 1 : y + r2) * W;
    const row = y * W;
    let o = row * 3;
    for (let x = 0; x < W; x++) {
      const li = row + x;
      ...
      const bF = far > 0.12 ? (far - 0.12) * 0.3 : 0;
      const addR = (bN * 0.2 + bF * 0.1) * 255;
      const addG = (bN * 0.42 + bF * 0.22) * 255;
      const addB = (bN * 0.55 + bF * 0.3) * 255;
