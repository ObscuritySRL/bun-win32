# AI Guide for @bun-win32/winhttp
...
```ts
import Winhttp, { SomeFlag } from '@bun-win32/winhttp';

// Methods bind lazily on first call
const result = Winhttp.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Winhttp.Preload(['SomeFunctionW', 'AnotherFunction']);
Winhttp.Preload('SomeFunctionW');
Winhttp.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Winhttp.ts` |
| Find types, enums, constants      | `types/Winhttp.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/winhttp` directly.

## Calling Convention

All documented `winhttp.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Winhttp.SomeFunctionW(wide.ptr);
