...
```ts
import Dcomp, { SomeFlag } from '@bun-win32/dcomp';

// Methods bind lazily on first call
const result = Dcomp.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dcomp.Preload(['SomeFunctionW', 'AnotherFunction']);
Dcomp.Preload('SomeFunctionW');
Dcomp.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dcomp.ts` |
| Find types, enums, constants      | `types/Dcomp.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dcomp` directly.

## Calling Convention

All documented `dcomp.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
