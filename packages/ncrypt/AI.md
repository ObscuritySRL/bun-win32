...
```ts
import Ncrypt, { SomeFlag } from '@bun-win32/ncrypt';

// Methods bind lazily on first call
const result = Ncrypt.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Ncrypt.Preload(['SomeFunctionW', 'AnotherFunction']);
Ncrypt.Preload('SomeFunctionW');
Ncrypt.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Ncrypt.ts` |
| Find types, enums, constants      | `types/Ncrypt.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/ncrypt` directly.

## Calling Convention

All documented `ncrypt.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
