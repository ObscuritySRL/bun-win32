# AI Guide for @bun-win32/gdiplus
...
```ts
import Gdiplus, { SomeFlag } from '@bun-win32/gdiplus';

// Methods bind lazily on first call
const result = Gdiplus.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Gdiplus.Preload(['SomeFunctionW', 'AnotherFunction']);
Gdiplus.Preload('SomeFunctionW');
Gdiplus.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Gdiplus.ts` |
| Find types, enums, constants      | `types/Gdiplus.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/gdiplus` directly.

## Calling Convention

All documented `gdiplus.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Gdiplus.SomeFunctionW(wide.ptr);
