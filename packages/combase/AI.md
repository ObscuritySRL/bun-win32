# AI Guide for @bun-win32/combase
...
```ts
import Combase, { SomeFlag } from '@bun-win32/combase';

// Methods bind lazily on first call
const result = Combase.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Combase.Preload(['SomeFunctionW', 'AnotherFunction']);
Combase.Preload('SomeFunctionW');
Combase.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Combase.ts` |
| Find types, enums, constants      | `types/Combase.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/combase` directly.

## Calling Convention

All documented `combase.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Combase.SomeFunctionW(wide.ptr);
