# AI Guide for @bun-win32/wintrust
...
```ts
import Wintrust, { SomeFlag } from '@bun-win32/wintrust';

// Methods bind lazily on first call
const result = Wintrust.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wintrust.Preload(['SomeFunctionW', 'AnotherFunction']);
Wintrust.Preload('SomeFunctionW');
Wintrust.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Wintrust.ts` |
| Find types, enums, constants      | `types/Wintrust.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/wintrust` directly.

## Calling Convention

All documented `wintrust.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Wintrust.SomeFunctionW(wide.ptr);
