...
```ts
import Mi, { SomeFlag } from '@bun-win32/mi';

// Methods bind lazily on first call
const result = Mi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mi.Preload(['SomeFunctionW', 'AnotherFunction']);
Mi.Preload('SomeFunctionW');
Mi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Mi.ts` |
| Find types, enums, constants      | `types/Mi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/mi` directly.

## Calling Convention

All documented `mi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
