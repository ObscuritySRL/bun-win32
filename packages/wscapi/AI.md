...
```ts
import Wscapi, { SomeFlag } from '@bun-win32/wscapi';

// Methods bind lazily on first call
const result = Wscapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wscapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Wscapi.Preload('SomeFunctionW');
Wscapi.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Wscapi.ts` |
| Find types, enums, constants      | `types/Wscapi.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/wscapi` directly.

## Calling Convention

All documented `wscapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
