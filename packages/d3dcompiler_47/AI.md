# AI Guide for @bun-win32/d3dcompiler_47
...
```ts
import D3dcompiler_47, { SomeFlag } from '@bun-win32/d3dcompiler_47';

// Methods bind lazily on first call
const result = D3dcompiler_47.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
D3dcompiler_47.Preload(['SomeFunctionW', 'AnotherFunction']);
D3dcompiler_47.Preload('SomeFunctionW');
D3dcompiler_47.Preload();
```
...
| Need                              | Read                        |
| --------------------------------- | --------------------------- |
| Find a method or its MS Docs link | `structs/D3dcompiler_47.ts` |
| Find types, enums, constants      | `types/D3dcompiler_47.ts`   |
| Quick examples                    | `README.md`                 |

`index.ts` re-exports the class and all types — import from `@bun-win32/d3dcompiler_47` directly.

## Calling Convention

All documented `d3dcompiler_47.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
D3dcompiler_47.SomeFunctionW(wide.ptr);
...
D3dcompiler_47.SomeFunction(out.ptr);
