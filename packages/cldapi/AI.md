...
```ts
import Cldapi, { SomeFlag } from '@bun-win32/cldapi';

// Methods bind lazily on first call
const result = Cldapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Cldapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Cldapi.Preload('SomeFunctionW');
Cldapi.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Cldapi.ts` |
| Find types, enums, constants      | `types/Cldapi.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/cldapi` directly.

## Calling Convention

All documented `cldapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
