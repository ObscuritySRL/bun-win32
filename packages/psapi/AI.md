...
```ts
import Psapi, { SomeFlag } from '@bun-win32/psapi';

// Methods bind lazily on first call
const result = Psapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Psapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Psapi.Preload('SomeFunctionW');
Psapi.Preload();
```
...
| Need                              | Read               |
| --------------------------------- | ------------------ |
| Find a method or its MS Docs link | `structs/Psapi.ts` |
| Find types, enums, constants      | `types/Psapi.ts`   |
| Quick examples                    | `README.md`        |

`index.ts` re-exports the class and all types — import from `@bun-win32/psapi` directly.

## Calling Convention

All documented `psapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
