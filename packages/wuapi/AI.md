...
```ts
import Wuapi, { SomeFlag } from '@bun-win32/wuapi';

// Methods bind lazily on first call
const result = Wuapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Wuapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Wuapi.Preload('SomeFunctionW');
Wuapi.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Wuapi.ts`  |
| Find types, enums, constants      | `types/Wuapi.ts`    |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/wuapi` directly.

## Calling Convention

All documented `wuapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
