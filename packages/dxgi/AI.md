...
```ts
import Dxgi, { SomeFlag } from '@bun-win32/dxgi';

// Methods bind lazily on first call
const result = Dxgi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dxgi.Preload(['SomeFunctionW', 'AnotherFunction']);
Dxgi.Preload('SomeFunctionW');
Dxgi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dxgi.ts` |
| Find types, enums, constants      | `types/Dxgi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dxgi` directly.

## Calling Convention

All documented `dxgi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
