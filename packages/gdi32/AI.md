# AI Guide for @bun-win32/gdi32

This file documents the binding contract of this package, not the Windows API itself.

## What This Package Is

- The package exports one class: `GDI32`.
- Every `GDI32.MethodName(...)` call is a thin 1:1 binding to `gdi32.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `GDI32.Preload()` only changes when symbols are bound. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `GDI32` class and re-exports everything from `types/GDI32.ts`.
- `structs/GDI32.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
- `types/GDI32.ts` contains the aliases, enums, constants, flags, and sentinels used by the method signatures.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/gdi32` when available.
