...
```ts
import GDI32, { SomeFlag } from '@bun-win32/gdi32';

// Methods bind lazily on first call
const result = GDI32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
GDI32.Preload(['SomeFunctionW', 'AnotherFunction']);
GDI32.Preload('SomeFunctionW');
GDI32.Preload();
```
...
| Need                              | Read               |
| --------------------------------- | ------------------ |
| Find a method or its MS Docs link | `structs/GDI32.ts` |
| Find types, enums, constants      | `types/GDI32.ts`   |
| Quick examples                    | `README.md`        |

`index.ts` re-exports the class and all types — import from `@bun-win32/gdi32` directly.

## Calling Convention

All documented `gdi32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
