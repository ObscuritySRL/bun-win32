...
```ts
import Avrt, { SomeFlag } from '@bun-win32/avrt';

// Methods bind lazily on first call
const result = Avrt.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Avrt.Preload(['SomeFunctionW', 'AnotherFunction']);
Avrt.Preload('SomeFunctionW');
Avrt.Preload();
```
...
| Need                              | Read              |
| --------------------------------- | ----------------- |
| Find a method or its MS Docs link | `structs/Avrt.ts` |
| Find types, enums, constants      | `types/Avrt.ts`   |
| Quick examples                    | `README.md`       |

`index.ts` re-exports the class and all types — import from `@bun-win32/avrt` directly.

## Calling Convention

All documented `avrt.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
