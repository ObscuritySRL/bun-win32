...
```ts
import D3d11, { SomeFlag } from '@bun-win32/d3d11';

// Methods bind lazily on first call
const result = D3d11.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
D3d11.Preload(['SomeFunctionW', 'AnotherFunction']);
D3d11.Preload('SomeFunctionW');
D3d11.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/D3d11.ts` |
| Find types, enums, constants      | `types/D3d11.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/d3d11` directly.

## Calling Convention

All documented `d3d11.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
