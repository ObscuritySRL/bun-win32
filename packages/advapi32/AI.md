...
```ts
import Advapi32, { SomeFlag } from '@bun-win32/advapi32';

// Methods bind lazily on first call
const result = Advapi32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Advapi32.Preload(['SomeFunctionW', 'AnotherFunction']);
Advapi32.Preload('SomeFunctionW');
Advapi32.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/Advapi32.ts`  |
| Find types, enums, constants      | `types/Advapi32.ts`    |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/advapi32` directly.

## Calling Convention

All documented `advapi32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Advapi32.SomeFunctionW(wide.ptr);
