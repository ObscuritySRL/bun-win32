# AI Guide for @bun-win32/ntdsapi
...
```ts
import Ntdsapi, { SomeFlag } from '@bun-win32/ntdsapi';

// Methods bind lazily on first call
const result = Ntdsapi.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Ntdsapi.Preload(['SomeFunctionW', 'AnotherFunction']);
Ntdsapi.Preload('SomeFunctionW');
Ntdsapi.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Ntdsapi.ts` |
| Find types, enums, constants      | `types/Ntdsapi.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/ntdsapi` directly.

## Calling Convention

All documented `ntdsapi.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Ntdsapi.SomeFunctionW(wide.ptr);
