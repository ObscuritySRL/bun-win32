# AI Guide for @bun-win32/shell32

This file documents the binding contract of this package, not the Windows API itself.

## What This Package Is

- The package exports one class: `Shell32`.
- Every `Shell32.MethodName(...)` call is a thin 1:1 binding to `shell32.dll!MethodName`.
- Method names, parameter names, and parameter order follow Microsoft Docs.
- `Shell32.Preload()` only changes when symbols are bound. It does not change signatures, return values, or semantics.
- The package does not add wrappers, helper objects, automatic conversions, error translation, or cleanup.

## How To Read It

- `index.ts` exports the default `Shell32` class and re-exports everything from `types/Shell32.ts`.
- `structs/Shell32.ts` is the method index. Each static method maps to one export and has a Microsoft Docs URL comment above it.
- `types/Shell32.ts` contains the aliases, enums, constants, flags, and sentinels used by the method signatures.
...
- Flags, enums, constants, and sentinels should be imported from `@bun-win32/shell32` when available.
