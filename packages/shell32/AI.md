...
```ts
import Shell32, { SomeFlag } from '@bun-win32/shell32';

// Methods bind lazily on first call
const result = Shell32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Shell32.Preload(['SomeFunctionW', 'AnotherFunction']);
Shell32.Preload('SomeFunctionW');
Shell32.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/Shell32.ts`   |
| Find types, enums, constants      | `types/Shell32.ts`     |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/shell32` directly.

## Calling Convention

All documented `shell32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Shell32.SomeFunctionW(wide.ptr);
