...
```ts
import Pdh, { SomeFlag } from '@bun-win32/pdh';

// Methods bind lazily on first call
const result = Pdh.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Pdh.Preload(['SomeFunctionW', 'AnotherFunction']);
Pdh.Preload('SomeFunctionW');
Pdh.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/Pdh.ts`       |
| Find types, enums, constants      | `types/Pdh.ts`         |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/pdh` directly.

## Calling Convention

All documented `pdh.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
