# AI Guide for @bun-win32/xinput1_4
...
```ts
import Xinput1_4, { SomeFlag } from '@bun-win32/xinput1_4';

// Methods bind lazily on first call
const result = Xinput1_4.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Xinput1_4.Preload(['SomeFunctionW', 'AnotherFunction']);
Xinput1_4.Preload('SomeFunctionW');
Xinput1_4.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/Xinput1_4.ts` |
| Find types, enums, constants      | `types/Xinput1_4.ts`   |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/xinput1_4` directly.

## Calling Convention

All documented `xinput1_4.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Xinput1_4.SomeFunctionW(wide.ptr);
...
Xinput1_4.SomeFunction(out.ptr);
