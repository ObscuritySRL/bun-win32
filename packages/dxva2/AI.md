...
```ts
import Dxva2, { SomeFlag } from '@bun-win32/dxva2';

// Methods bind lazily on first call
const result = Dxva2.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dxva2.Preload(['SomeFunctionW', 'AnotherFunction']);
Dxva2.Preload('SomeFunctionW');
Dxva2.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dxva2.ts` |
| Find types, enums, constants      | `types/Dxva2.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dxva2` directly.

## Calling Convention

All documented `dxva2.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
