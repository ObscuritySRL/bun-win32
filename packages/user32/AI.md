...
```ts
import User32, { SomeFlag } from '@bun-win32/user32';

// Methods bind lazily on first call
const result = User32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
User32.Preload(['SomeFunctionW', 'AnotherFunction']);
User32.Preload('SomeFunctionW');
User32.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/User32.ts` |
| Find types, enums, constants      | `types/User32.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/user32` directly.

## Calling Convention

All documented `user32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
