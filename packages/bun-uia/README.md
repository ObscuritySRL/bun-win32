...
- **MCP server for Claude** — `claude mcp add uia -- bunx bun-uia` exposes the whole surface as 59 policy-gated tools (53 under the default `safe` profile); the agent drives Windows cursor-free, sees the desktop, and (when enabled) launches apps and reads/writes files.
