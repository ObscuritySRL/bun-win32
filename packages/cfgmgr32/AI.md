# AI Guide for @bun-win32/cfgmgr32
...
```ts
import Cfgmgr32, { SomeFlag } from '@bun-win32/cfgmgr32';

// Methods bind lazily on first call
const result = Cfgmgr32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Cfgmgr32.Preload(['SomeFunctionW', 'AnotherFunction']);
Cfgmgr32.Preload('SomeFunctionW');
Cfgmgr32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Cfgmgr32.ts` |
| Find types, enums, constants      | `types/Cfgmgr32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/cfgmgr32` directly.

## Calling Convention

All documented `cfgmgr32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Cfgmgr32.SomeFunctionW(wide.ptr);
