# AI Guide for @bun-win32/oleaut32
...
```ts
import Oleaut32, { SomeFlag } from '@bun-win32/oleaut32';

// Methods bind lazily on first call
const result = Oleaut32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Oleaut32.Preload(['SomeFunctionW', 'AnotherFunction']);
Oleaut32.Preload('SomeFunctionW');
Oleaut32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Oleaut32.ts` |
| Find types, enums, constants      | `types/Oleaut32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/oleaut32` directly.

## Calling Convention

All documented `oleaut32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Oleaut32.SomeFunctionW(wide.ptr);
