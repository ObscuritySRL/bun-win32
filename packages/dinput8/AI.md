# AI Guide for @bun-win32/dinput8
...
```ts
import Dinput8, { SomeFlag } from '@bun-win32/dinput8';

// Methods bind lazily on first call
const result = Dinput8.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Dinput8.Preload(['SomeFunctionW', 'AnotherFunction']);
Dinput8.Preload('SomeFunctionW');
Dinput8.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Dinput8.ts` |
| Find types, enums, constants      | `types/Dinput8.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/dinput8` directly.

## Calling Convention

All documented `dinput8.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Dinput8.SomeFunctionW(wide.ptr);
