# AI Guide for @bun-win32/mfreadwrite
...
```ts
import Mfreadwrite, { SomeFlag } from '@bun-win32/mfreadwrite';

// Methods bind lazily on first call
const result = Mfreadwrite.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Mfreadwrite.Preload(['SomeFunctionW', 'AnotherFunction']);
Mfreadwrite.Preload('SomeFunctionW');
Mfreadwrite.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Mfreadwrite.ts` |
| Find types, enums, constants      | `types/Mfreadwrite.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/mfreadwrite` directly.

## Calling Convention

All documented `mfreadwrite.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Mfreadwrite.SomeFunctionW(wide.ptr);
...
Mfreadwrite.SomeFunction(out.ptr);
