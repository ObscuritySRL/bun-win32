...
```ts
import Quartz, { SomeFlag } from '@bun-win32/quartz';

// Methods bind lazily on first call
const result = Quartz.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Quartz.Preload(['SomeFunctionW', 'AnotherFunction']);
Quartz.Preload('SomeFunctionW');
Quartz.Preload();
```
...
| Need                              | Read                |
| --------------------------------- | ------------------- |
| Find a method or its MS Docs link | `structs/Quartz.ts` |
| Find types, enums, constants      | `types/Quartz.ts`   |
| Quick examples                    | `README.md`         |

`index.ts` re-exports the class and all types — import from `@bun-win32/quartz` directly.

## Calling Convention

All documented `quartz.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
