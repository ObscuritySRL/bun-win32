...
```ts
import Oleacc, { SomeFlag } from '@bun-win32/oleacc';

// Methods bind lazily on first call
const result = Oleacc.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Oleacc.Preload(['SomeFunctionW', 'AnotherFunction']);
Oleacc.Preload('SomeFunctionW');
Oleacc.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Oleacc.ts` |
| Find types, enums, constants      | `types/Oleacc.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/oleacc` directly.

## Calling Convention

All documented `oleacc.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
