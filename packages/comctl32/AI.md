# AI Guide for @bun-win32/comctl32
...
```ts
import Comctl32, { SomeFlag } from '@bun-win32/comctl32';

// Methods bind lazily on first call
const result = Comctl32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
Comctl32.Preload(['SomeFunctionW', 'AnotherFunction']);
Comctl32.Preload('SomeFunctionW');
Comctl32.Preload();
```
...
| Need                              | Read                 |
| --------------------------------- | -------------------- |
| Find a method or its MS Docs link | `structs/Comctl32.ts` |
| Find types, enums, constants      | `types/Comctl32.ts`   |
| Quick examples                    | `README.md`          |

`index.ts` re-exports the class and all types — import from `@bun-win32/comctl32` directly.

## Calling Convention

All documented `comctl32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
...
Comctl32.SomeFunctionW(wide.ptr);
