...
```ts
import GLU32, { SomeFlag } from '@bun-win32/glu32';

// Methods bind lazily on first call
const result = GLU32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
GLU32.Preload(['SomeFunctionW', 'AnotherFunction']);
GLU32.Preload('SomeFunctionW');
GLU32.Preload();
```
...
| Need                              | Read                   |
| --------------------------------- | ---------------------- |
| Find a method or its MS Docs link | `structs/GLU32.ts`     |
| Find types, enums, constants      | `types/GLU32.ts`       |
| Quick examples                    | `README.md`            |

`index.ts` re-exports the class and all types — import from `@bun-win32/glu32` directly.

## Calling Convention

All documented `glu32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
