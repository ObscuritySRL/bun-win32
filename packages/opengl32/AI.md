...
```ts
import OpenGL32, { SomeFlag } from '@bun-win32/opengl32';

// Methods bind lazily on first call
const result = OpenGL32.SomeFunctionW(arg1, arg2);

// Preload: array, single string, or no args (all symbols)
OpenGL32.Preload(['SomeFunctionW', 'AnotherFunction']);
OpenGL32.Preload('SomeFunctionW');
OpenGL32.Preload();
```
...
All documented `opengl32.dll` exports are bound. Each method maps 1:1 to its DLL export. Names, parameter names, and order match Microsoft Docs.
