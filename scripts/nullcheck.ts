...
function headerKind(cTypeRaw: string): 'pointer' | 'handle' | 'scalar' | 'unknown' {
  ...
  if (/(?:^|_)HANDLE$/.test(type)) return 'handle'; // *_HANDLE opaque typedefs
  if (SCALAR_CTYPES.has(type)) return 'scalar';
  if (DATA_POINTER_ALIASES.has(type)) return 'pointer';
  if (/^LP[A-Z]/.test(type)) return 'pointer'; // LP* is reliably a pointer
  if (/^H[A-Z][A-Z0-9_]*$/.test(type)) return 'handle';
  return 'unknown'; // exotic P*/struct names: stay conservative, don't guess
}
...
function isDataPointer(cTypeRaw: string): boolean {
  const type = cTypeRaw.trim();
  if (SCALAR_CTYPES.has(type)) return false; // LPARAM/LRESULT etc. are LONG_PTR ints, not pointers
  if (type.endsWith('*')) {
    const base = type
      .replace(/\*+$/, '')
  ...
  if (/^LP[A-Z]/.test(type)) return true; // LPxxx alias (LPVOID, LPWSTR, LPSTARTUPINFOW, ...)
  return false;
}
...
function buildSdkIndex(functionNames: string[]): void {
  ...
  try {
    ...
    // Collect EVERY (header, line) a function name appears on, scored so a real
    // prototype (name at line start, or name + linkage macro) outranks a comment
    // or macro mention. Search across all matching headers, not just the first.
    ...
    for (const rawLine of grepOutput.split('\n')) {
      ...
      // Pick the identifier-before-paren that is actually a bound symbol. The first
      // `\w+(` is often a return-type macro: `LWSTDAPI_(BOOL) Foo(`, `STDAPI_(x) Bar(`.
      let functionName = '';
      ...
      hits.get(functionName)!.push({ path: lineMatch[1], line: parseInt(lineMatch[2], 10) - 1, score });
    }
    ...
    for (const [functionName, candidates] of hits) {
      ...
      for (const candidate of candidates) {
        ...
        // Accumulate lines until the prototype's own parens balance. Anchor on the
        // `(` immediately after the function name — NOT the first `(` in the block:
        // `LWSTDAPI_(BOOL) Foo(...)` / `STDAPI_(x) Bar(...)` carry the return type in
        // a macro paren BEFORE the name.
        let block = '';
...
function parseSdkParams(inner: string): SdkParam[] {
  ...
  for (const rawPart of splitTopLevel(inner)) {
    ...
    const cleaned = part
      ...
      .replace(/\s+/g, ' ')
      .trim();
    // The parameter name is the last identifier; everything before it is the
    // (SAL-stripped) C type. Collapse `Type *name` / `Type* name` to `Type*`.
...
function auditPackage(pkg: string): { findings: Finding[]; className: string } {
  ...
  for (const method of methods) {
    ...
    // Confidence gate: only trust this prototype for decisions when a strong
    // majority of the method's param names appear in it. Low overlap means the
    // parser likely matched a usage example / inline body, not the real decl —
    // route those to UNMATCHED instead of risking a false fix.
    ...
    for (const param of pointerHandleParams) {
      ...
      // Genuine pointer-vs-handle bug, not the sanctioned opaque-pointer->u64
      // convention. Guards: same type name on both sides is correct by definition
      // (P*_HANDLE etc.); an array param decays to a pointer; flag pointer-vs-handle
      // only against the explicit handle set or an unambiguous DATA pointer.
...
function applyMissingFixes(pkg: string, className: string, findings: Finding[]): { applied: number; skipped: Finding[] } {
  ...
  for (const [lineNumber, lineFindings] of byLine) {
    ...
    for (const finding of lineFindings) {
      // Replace `name: BaseType` followed by `,` or `)`, not already followed by `|`.
...
const totals = { missing: 0, spurious: 0, unmatched: 0, noSdk: 0, mismatch: 0, applied: 0 };
...
// Gate: fail on definite bugs (MISSING), and on TYPE_MISMATCH only under --strict.
