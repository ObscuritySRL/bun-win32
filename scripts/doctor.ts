...
function checkPlatform(): void {
  if (process.platform === 'win32') {
    checkResults.push({ label: 'Platform', status: 'pass', detail: `${process.platform} (${process.arch})` });
    return;
  }
...
function checkBunVersion(): void {
  ...
  if (!versionMatch) {
    checkResults.push({ label: 'Bun version', status: 'warn', detail: `could not parse "${bunVersionString}"` });
    return;
  }
...
function checkRipgrep(): void {
  ...
  try {
    ...
    checkResults.push({ label: 'ripgrep (rg)', status: 'pass', detail: `${firstLine} — ${candidateCommand}` });
  } catch {
    checkResults.push({
      label: 'ripgrep (rg)',
      status: 'fail',
...
function checkDumpbin(): void {
  ...
  checkResults.push({ label: 'bin/dumpbin.exe', status: 'pass', detail: dumpbinPath });
}
...
function checkRepositoryLayout(): void {
  ...
  checkResults.push({ label: 'Repository layout', status: 'pass', detail: 'all required scripts and template present' });
}
...
function checkRepositoryInstall(): void {
  ...
  checkResults.push({ label: 'Root install', status: 'pass', detail: 'node_modules present at repo root' });
}
